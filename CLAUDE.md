...
- `overlay_logic.py` — Copernicus Marine overlays (chlorophyll, temp, salinity, O2, pH, biomass, SSH) and surface currents; requires local `copernicusmarine login`. Each Copernicus dataset is opened once (lazy xarray handle, 1h TTL) and sliced per request — `copernicusmarine.open_dataset` costs ~3s of auth + catalogue round-trips *before* any data, so never call it per request.
- `overlay_prefetch.py` — background worker that fetches all overlay layers for every READY file once (queued at the end of `cache_logic._process`) and stores them in the per-file binary cache (`put_plot_binary`, key `overlay:<var>`), so a layer click is a disk read. Finished deployments are fetched once for life; live gliders (last fix within 7 days) are re-checked every 5 min against the dataset's latest day and only re-downloaded when a newer day exists. `/api/overlay_status` drives the map view's greyed-out layer buttons; invalidation rides on `clear_plot_binary` (file changed/deleted/version bump).
- `argo_logic.py` — **experimental, self-contained** Argo float explorer: reduces the GDAC global profile index (daily download to `~/.glider_playground/argo/`) to last-position-per-float (plus mmapped per-profile lat/lon/date/WMO `.npy` arrays for box+time queries) and proxies Euro-Argo fleet-monitoring details. Only touched via three `/api/argo/*` routes in `app.py`, three one-line `ArgoLayer` hooks in `map_view.html` (script tag, attach, click/hover; the layer itself is `static/argo_layer.js`), and the 3D view's `/api/argo/profiles` fetch that shows floats surfacing near the glider during playback. Remove those and it's gone. No profile data is downloaded.
- `ships_logic.py` — **experimental, self-contained** research-ship positions: scrapes the GeoJSON `data-geojson` block on NOC's ships-and-expeditions page (daily bridge reports for RRS Discovery / James Cook) and BAS's hourly `sda.pos` met-report file (RRS Sir David Attenborough). Latest fix only — both feeds are only good to a few km, so no tracks. No AIS provider, no keys; cached 30 min in-process. One `/api/ships` route in `app.py`, four one-line `ShipsLayer` hooks in `map_view.html` (ships render through the map's own HTML marker layer, same as glider end markers), layer in `static/ships_layer.js`; icons vendored in `static/icons/`. Remove those and it's gone.
...
- `3d_view.html` — the 3D dive-track + bathymetry view; a singleton panel (only one instance, unlike plots/maps which can multiply). Its own assets live in `3d_view/`: `models/` (low-poly vehicle/object JSON meshes) and `scenery/` (decorative life behind the default-on "Scenery" style toggle, which also drives the seabed depth shading — `scenery.js` + meshes + `make_models.py` generator; cosmetic only, see `3d_view/README.md`).
