...
- `argo_logic.py` — **experimental, self-contained** Argo float explorer: reduces the GDAC global profile index (daily download to `~/.glider_playground/argo/`) to last-position-per-float and proxies Euro-Argo fleet-monitoring details. Only touched via two `/api/argo/*` routes in `app.py` and three one-line `ArgoLayer` hooks in `map_view.html` (script tag, attach, click/hover); the layer itself is `static/argo_layer.js`. Remove those and it's gone. No profile data is downloaded.
