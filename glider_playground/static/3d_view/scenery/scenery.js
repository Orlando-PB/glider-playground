// Scenery for the 3D view: decorative low-poly life placed by RULES from a seeded RNG (same file,
...
window.Scenery = (() => {
    ...
    // upright: grows straight up on a slope (else tilted flush). sway: underwater motion strength (0 = rigid).
    // swim: { r, period, bob } — stateless elliptical loop of radius ~r (scene units) per `period` seconds.
    // snow: 'coat' blends towards snow, 'skip' places nothing (see snowAt).
    ...
    // `group` caps the total across species (GROUP_MAX); species order is shuffled per file.
    ...
    const RULES = [
        // ── Standing things (split by biogeography via `region` boxes) ──
        // Giant kelp: Pacific Americas and the cold-temperate southern hemisphere; not the N Atlantic.
        ...
        // Conifers: temperate and boreal land up to the Arctic treeline, south to Tierra del Fuego; snow-covered where the land is drawn white.
        { model: 'conifer',  land: true,      region: [[-180, 180, 28, 70], [-180, 180, -56, -28]], size: 0.025, density: 0.4, max: 60, snow: 'coat' },
        // Shrubs: any land up to the tundra shrub line (none in Antarctica), but not on snow.
        { model: 'bush',     land: true,      region: [[-180, 180, -56, 68]], size: 0.008, density: 0.5, max: 120, snow: 'skip' },
    ...
    const LAND_SLOTS = 400;         // plant spots an all-land scene would hold (one per ~0.05 x 0.05 scene units)
    ...
    // Tallest thing that can stand on land (scene units); the view reserves this headroom.
    ...
    function build(data, bathyZ, S, seed, snowAt, floorZ) {
        ...
        for (const rule of order) {
            ...
            // At least one wherever there's any suitable ground at all.
            let want = Math.min(Math.ceil(rule.max * ABUNDANCE), Math.ceil(cells.length * rule.density * ABUNDANCE));
            // Land is counted by its share of the scene, not by cells: a sliver of coast gets few plants or none.
            if (rule.land) want = Math.min(want, Math.floor(cells.length / (nx * ny) * LAND_SLOTS * rule.density * ABUNDANCE));
            if (rule.group) want = Math.min(want, groupCap(rule.group) - (groupCount[rule.group] || 0));
            // A pick can fail, so keep drawing cells until `want` are placed.
            let placed = 0;
            for (let q = 0; q < cells.length && placed < want && q < want * TRIES_PER_INSTANCE; q++) {
                ...
                // Anchor height: the ground for standers; a random depth in the band for floaters.
                let anchor = ground;
                if (rule.surface) {
                ...
                // Keep the whole footprint inside the box (the axis range slices meshes open);
                // swimmers near the edge get a tighter loop.
                let loopScale = 1;
                if (rule.swim) {
                ...
                // Seabed things tilt onto the local surface normal (in scene units) and sink a touch; `upright` kinds don't.
                let tilt = null;
