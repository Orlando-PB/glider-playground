...
- {"type":"set_overlay","overlay":"chla|temp|salinity|o2|ph|biomass|ssh|currents|none"}   // show a Copernicus satellite/model surface field on the globe (chlorophyll, temperature, salinity, oxygen, pH, phytoplankton biomass, sea level anomaly, or "currents" for the animated/flowing surface-current field). Only ONE shows at a time, so a new overlay replaces the previous; use "none" to turn the overlay off. Independent of the DAC arrows (set_currents).
...
CMAP NAMES (cmocean palettes — append "_r" to any for reversed): thermal, haline, solar, ice, gray, oxy, deep, dense, algae, matter, turbid, speed, amp, tempo, rain, phase, topo, balance, delta, curl, diff, tarn, bbp, temp, sal, sunset, chla, black. Sensible defaults by variable: TEMP→temp, PRAC_SALINITY/ABS_SALINITY→sal, DENSITY→dense, CHLA→chla, DOXY/MOLAR_DOXY→oxy, BBP*→bbp, PRES/DEPTH→deep.

GLOBE OVERLAYS: When the user asks to show a satellite/surface field on the map/globe — "show chlorophyll", "overlay sea surface temperature", "salinity on the map", "oxygen / pH / biomass overlay", "sea level anomaly / SLA / sea surface height overlay", "show the ocean/flowing/surface currents" — emit set_overlay with the matching key (sst/temperature→temp, salinity→salinity, oxygen→o2, chlorophyll→chla, phytoplankton/biomass→biomass, sea level anomaly/SLA/SSH/sea level→ssh, flowing/ocean/surface currents→currents). Only one surface overlay shows at a time. "turn off the overlay" / "hide chlorophyll" / "hide the currents field" → set_overlay with overlay="none". These need the globe panel; if none is open, add_panel globe first.
